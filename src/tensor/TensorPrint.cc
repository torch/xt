...
static std::tuple<double, int64_t> __printFormat(std::ostream& stream, Tensor& self)
{
  auto size = numel(self);
  if(size == 0) {
...
static void __printMatrix(std::ostream& stream, Tensor& self, int64_t linesize, int64_t indent)
{
  double scale;
  int64_t sz;
...
void __printTensor(std::ostream& stream, Tensor& self, int64_t linesize)
{
  ...
  while(true) {
    ...
    stream << "(";
    Tensor tensor = self;
    for(int64_t i=0; i < self.dim()-2; i++) {
      tensor = select(tensor, 0, counter[i]);
      stream << counter[i]+1 << ",";
    }
    stream << ".,.) = " << std::endl;
...
std::ostream& Tensor::print(std::ostream& stream, int64_t linesize) const
{
  if(device_ == kUnknown) {
    stream << "[Tensor<" << typedesc() << "," << devicedesc() << ">]";    
  } else if(dim() == -1) {
    stream << "[Tensor<" << typedesc() << "," << devicedesc() << "> (empty)]";
  } else {
    Tensor tensor = this->cast<double>(kCPU);
    if(tensor.dim() == 0) {
      stream << std::defaultfloat << tensor.data<double>()[0];
//      stream << "[Tensor<" << typedesc() << "," << devicedesc() << "> (value)]";
      ...
      stream << "[Tensor<" << typedesc() << "," << devicedesc() << "> of size " << tensor.size(0) << "]";
    } else if(tensor.dim() == 2) {
      __printMatrix(stream, tensor, linesize, 0);
      stream << "[Tensor<" << typedesc() << "," << devicedesc() << "> of size " << tensor.size(0) << "x" <<  tensor.size(1) << "]";
    } else {
